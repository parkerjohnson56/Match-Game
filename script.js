...
function shuffleCardImageClasses() {
   ...
   */
  const shuffledArray = _.shuffle(cardClasses);
...
function createCards(parentElement, shuffledImageClasses) {
  ...
  for (let i = 0; i < 12; i++) {
    ...
    objectsArray.push({
      index: i, // Loop index, e.g., 0, 1, 2...
      element: newCard, // The DOM element for the card
      imageClass: shuffledImageClasses[i], // Image class, e.g., "image-1"
    });
  }
...
/* The 'onCardFlipped' function below will be called each time the user flips a card. 
The 'lastCardFlipped' variable is used to remember the first card flipped while we wait 
for the user to flip another card. We need to keep track of this value to determine if the
 two cards flipped match or not. 'lastCardFlipped' should be reset to 'null' each time
  a second card is flipped. */
